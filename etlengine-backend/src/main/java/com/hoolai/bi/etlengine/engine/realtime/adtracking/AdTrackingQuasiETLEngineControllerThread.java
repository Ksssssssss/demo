...
public class AdTrackingQuasiETLEngineControllerThread implements Runnable{
	
	private static final Logger LOGGER=Logger.getLogger(AdTrackingQuasiETLEngineControllerThread.class.getSimpleName());
	...
	public AdTrackingQuasiETLEngineControllerThread(String snid, String gameid, String ds,Integer maxThreadNum,
	...
	public void run() {
		...
		try {
			ETLEngineGameInfo etlEngineGameInfo=new ETLEngineGameInfo(Type.ADTRACKING_QUASI_ETL_ENGINE_RUN.getDisplayName(),snid, gameid, ds, step);
			TaskStatus taskStatus=this.taskStatusService.getTaskStatusByCode(AdTrackingQuasiETLEngineProcesser.TASK_STATUS_NAME,etlEngineGameInfo.getIdentification());
			...
			if(isSucc){
				...
				SMS_SENDER_PROXY.send(new SmsMess(46,Constant.MONITOR_ETL_PHONES,new String[]{"准实时(Adtracking)","失败",this.snid,this.gameid,this.ds,this.step+""}));
			}
	...
	private boolean runFor(int level,List<EngineEtls> elEngineEtls){
		...
		return this.runForEngineEtls("Adtracking-Quasi level:"+level, elEngineEtls);
