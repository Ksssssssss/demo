...
@Component
public class QuasiETLEngineProcesser {
	
	private static final Logger logger=Logger.getLogger(QuasiETLEngineProcesser.class.getSimpleName());
	
	private static final Logger ERROR_LOGGER=Logger.getLogger("exception");
	
	public static final String TASK_STATUS_NAME=Type.QUASI_ETL_ENGINE_RUN.getDisplayName();
	...
	public void processGames(List<ETLEngineGameInfo> etlEngineGameInfos){
		...
		List<EngineEtls> engineEtls = this.engineEtlsService.getEngineEtlsByType(EngineEtlsAdapter.Types.QUASI.name().toLowerCase());
			...
			try {
				...
				if(TerminalType.MOBILE_PHONE.equals(terminalType)){
					if(GameMobRunSystemType.IOS.getId()==etlEngineGameInfo.getSystemType()){
						levelEngineEtlsMap.putAll(getEengineEtls(engineEtls, terminalType.getId(), GameMobRunSystemType.IOS.getId()));
					}else{
						// 默认走pc的
						levelEngineEtlsMap.putAll(getEengineEtls(engineEtls, TerminalType.PC.getId(), GamePcRunSystemType.DEFAULT.getId()));
					}
				}else{
					levelEngineEtlsMap.putAll(getEengineEtls(engineEtls, terminalType.getId(), GamePcRunSystemType.DEFAULT.getId()));
				}
				
				
				GameQuasiETLEngineControllerThread gameETLEngineControllerThread=new GameQuasiETLEngineControllerThread(etlEngineGameInfo.getSnid(), etlEngineGameInfo.getGameid(), etlEngineGameInfo.getDs(), 
	...
	private Map<Integer,List<EngineEtls>> getEengineEtls(List<EngineEtls> engineEtls, Integer terminalType, Integer systemType){
		...
		for (EngineEtls engineEtl : engineEtls) {
			...
			if(null != engineEtl.getTerminalType() && terminalType.intValue() == engineEtl.getTerminalType().intValue() && null != engineEtl.getSystemType() && systemType.intValue() == engineEtl.getSystemType().intValue()){
	...
	private static class AsyncETLEngineTrigger implements Runnable{
		...
		private final QuasiETLEngineProcesser engineProcesser;
		
		private final List<List<ETLEngineGameInfo>> etlEngineGameInfos;

		public AsyncETLEngineTrigger(QuasiETLEngineProcesser engineProcesser, List<List<ETLEngineGameInfo>> etlEngineGameInfos) {
			super();
